...
def get_arg():
    ...
    cfg.add_argument('--k_shot',default=3,type=int)
...
def main(cfg):
    ...
    model = ThreeD_Support_Net(n_way=cfg.n_way, k_shot=cfg.k_shot, query=cfg.query, 
                                    backbone=cfg.backbone, fs=cfg.fs_head, 
                                    prj_num=cfg.prj_num, pretrain=cfg.pretrain, 
...
def test_model(model,val_loader,cfg):
    ...
    # epoch_to_test = epoch
    # target_pth_file = os.path.join(pth_fold, f'epoch_{epoch_to_test}')
...
def train_model(model,train_loader,val_loader,cfg):
    ...
    for e in tqdm_epochs: 
        ...
        if hasattr(model, 'fs_head') and hasattr(model.fs_head, 'epoch'):
            model.fs_head.epoch = e
        ...
        # print('epoch {}: {}. Highest: {}'.format(e,accuracy,np.max(acc_list)))

        if np.max(acc_list)==acc_list[-1]:
            summary_saved={**summary,'model_state':model.state_dict(),'optimizer_state':optimizer.state_dict(),'cfm':conf_mat,'batch_acclist':batch_acc_list}
